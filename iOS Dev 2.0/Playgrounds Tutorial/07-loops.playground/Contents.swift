...
var transactions: [Double] = [10.0, 450.0,1.0,54.9,20.7,48.8,17.9,57.9,789.0,47.3]

for i in 0...9 {
    print("Purchase: $\(transactions[i])")
}

var y = 0

repeat {
    transactions[y] = transactions[y] + 0.1
    print("Purchase wiht tax: $\(transactions[y])")
    y += 1
} while y < 10

var k = transactions.count - 1

repeat {
    print("Purchase: $\(transactions[k])")
    k -= 1
} while k  >= 0
